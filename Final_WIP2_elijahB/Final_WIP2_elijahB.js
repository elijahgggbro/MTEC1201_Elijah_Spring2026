...
//This is my second Prototype of my final project: the Electronic
...
//  This DAW will include an 8-beat step drum sequencer, inspired
//  by the Linn-drum machine. The user will be able to program a drum
//  pattern, change tempo, and gain control.



// In this iteration, I added functionality to the drum sequencer, as well as a playable synth using your 
// computer keyboard, and a BeePM( bees per minute) slider.  Played notes will display in the work in
// progress synth box(visuals will be added later on).

// I was having issues with making the notes play consistently, which will be improved in the next iteration.
// Design is not final. 
// If I had more time I would really like to add more beat steps, as well as a piano roll.

// effect knobs possibly coming in next iteration.
...
function keyPressed() {
  ...
  if (key === 'a') {sawOsc.freq(130); noteDisplay = 'C3'  } // C
  if (key === 's') {sawOsc.freq(145); noteDisplay = 'D'  }// D
  if (key === 'd') {sawOsc.freq(164); noteDisplay = 'E'  }// E
  if (key === 'f') {sawOsc.freq(174); noteDisplay = 'F'  }// F
  if (key === 'g') {sawOsc.freq(196); noteDisplay = 'G'  }// G
  if (key === 'h') {sawOsc.freq(220); noteDisplay = 'A'  }// A
  if (key === 'j') {sawOsc.freq(246); noteDisplay = 'B'  } // B
  if (key === 'k') {sawOsc.freq(261); noteDisplay = 'C4'  }  // C

  //accidentals (Q-Y)
  if (key === 'q') {sawOsc.freq(138); noteDisplay = 'C#3' } // C#
  if (key === 'w') {sawOsc.freq(155); noteDisplay = 'D#' } // D#
  if (key === 'e') {sawOsc.freq(185); noteDisplay = 'F#' } // F#
  if (key === 'r') {sawOsc.freq(207); noteDisplay = 'G#' } // G#
  if (key === 't') {sawOsc.freq(233); noteDisplay = 'A#' }// A#
  if (key === 'y') {sawOsc.freq(277); noteDisplay = 'C#4' } // C#
...
fill(100);
textSize(19);
text("Version 1.1",width/2+10,115);
